    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      ...
      <div style={{ position: 'relative', background: 'var(--color-card)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-xl)', width: '100%', maxWidth: 440, padding: '1.5rem', zIndex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
          <h3 style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: '1.05rem', margin: 0 }}>
            Update Status
          </h3>
        ...
        <div style={{ marginBottom: '1rem', padding: '0.75rem 1rem', borderRadius: 'var(--radius-md)', background: actionCfg?.bg, border: `1px solid ${actionCfg?.color}30` }}>
          <p style={{ margin: 0, fontSize: '0.875rem', fontWeight: 600, color: actionCfg?.color }}>
            Marking &ldquo;{redemption.rewardSnapshot?.name}&rdquo; as <strong>{actionCfg?.label}</strong>.
          </p>
          <p style={{ margin: '0.25rem 0 0', fontSize: '0.78rem', color: 'var(--color-body)' }}>
        ...
        {needsTracking && (
          <div style={{ marginBottom: '0.875rem' }}>
            <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, marginBottom: '0.375rem', color: 'var(--color-heading)' }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '0.625rem 0.875rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'var(--color-card)', fontSize: '0.875rem', color: 'var(--color-heading)', outline: 'none', boxSizing: 'border-box' }}
            />
          </div>
        )}
        ...
        <div style={{ marginBottom: '1.25rem' }}>
          ...
          <textarea
            ...
            style={{ width: '100%', padding: '0.625rem 0.875rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'var(--color-card)', fontSize: '0.875rem', color: 'var(--color-heading)', outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }}
          />
        </div>

        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <button onClick={onClose} disabled={loading} style={{ flex: 1, padding: '0.75rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'transparent', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer' }}>
            Cancel
          </button>
          <button
...
const AddressCell = ({ address }) => {
  ...
  if (!address?.line1) return <span style={{ color: 'var(--color-body)', fontSize: '0.78rem' }}>—</span>;
  return (
    <div style={{ position: 'relative' }}>
      <button
        onClick={() => setOpen(!open)}
        style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', background: 'rgba(37,99,235,0.07)', border: '1px solid rgba(37,99,235,0.2)', borderRadius: '6px', padding: '0.25rem 0.625rem', fontSize: '0.75rem', fontWeight: 600, color: 'var(--color-primary)', cursor: 'pointer' }}
      >
        <MapPin size={12} /> View
      </button>
      {open && (
        <>
          ...
          <div style={{ position: 'absolute', top: 'calc(100% + 0.5rem)', left: 0, zIndex: 20, background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-lg)', padding: '0.875rem 1rem', minWidth: 220, fontSize: '0.8rem', lineHeight: 1.7 }}>
...
const StatTile = ({ label, value, color }) => (
  <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', border: '1px solid var(--color-border)', padding: '1rem 1.25rem', flex: '1 1 120px', minWidth: 0, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
    <div style={{ fontSize: '0.65rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--color-body)', marginBottom: '0.35rem' }}>{label}</div>
    <div style={{ fontSize: '1.5rem', fontWeight: 800, fontFamily: 'var(--font-heading)', color: color || 'var(--color-heading)', lineHeight: 1 }}>{value ?? 0}</div>
  </div>
);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F7F4', fontFamily: 'var(--font-primary)', padding: 'clamp(1rem,3vw,2rem) clamp(1rem,3vw,2rem) 3rem' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        {/* Page heading */}
        <div>
          <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(1.4rem,3vw,1.85rem)', fontWeight: 800, color: 'var(--color-heading)', margin: '0 0 0.35rem' }}>
            Reward Redemptions
          </h1>
        ...
        <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', border: '1px solid var(--color-border)', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', padding: '1.25rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '0.65rem 2.5rem 0.65rem 2.25rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'var(--color-card)', fontSize: '0.875rem', color: 'var(--color-heading)', outline: 'none', boxSizing: 'border-box' }}
            />
              ...
              return (
                <div key={r._id} style={{ background: 'white', borderRadius: 'var(--radius-lg)', border: '1px solid var(--color-border)', padding: '1.25rem 1.5rem', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center' }}>
                  ...
                  <div style={{ flex: '1 1 200px', minWidth: 0 }}>
                    <div style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--color-heading)', marginBottom: '0.2rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {r.rewardSnapshot?.name || '—'}
                    </div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.625rem', alignItems: 'center' }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.78rem', color: 'var(--color-body)' }}><User size={12} /> {r.user?.name || '—'}</span>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.78rem', color: 'var(--color-body)' }}><Coins size={12} /> {r.totalCoinsDeducted?.toLocaleString()} coins</span>
                      {r.quantity > 1 && <span style={{ fontSize: '0.78rem', color: 'var(--color-body)' }}>Qty: {r.quantity}</span>}
                      <span style={{ fontSize: '0.72rem', color: 'var(--color-body)' }}>{date}</span>
                    </div>
                    <div style={{ marginTop: '0.3rem', fontSize: '0.7rem', color: 'var(--color-body)', fontFamily: 'monospace' }}>{r.redemptionId}</div>
                  </div>
                  ...
                  {/* Tracking */}
                  {r.trackingNumber ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.78rem', color: '#8B5CF6', fontWeight: 600 }}>
        ...
        {totalPages > 1 && (
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.75rem' }}>
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} style={{ padding: '0.5rem 1rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'white', fontSize: '0.875rem', fontWeight: 600, cursor: page === 1 ? 'not-allowed' : 'pointer', opacity: page === 1 ? 0.5 : 1 }}>
              Previous
            </button>
            <span style={{ fontSize: '0.875rem', color: 'var(--color-body)', fontWeight: 500 }}>
              Page {page} of {totalPages}
            </span>
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} style={{ padding: '0.5rem 1rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'white', fontSize: '0.875rem', fontWeight: 600, cursor: page === totalPages ? 'not-allowed' : 'pointer', opacity: page === totalPages ? 0.5 : 1 }}>
              Next
            </button>
          </div>
        )}
